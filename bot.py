...
async def add_to_cart(callback: CallbackQuery):
    # Формат: add_to_cart_{category_key}_{item_id}
    parts = callback.data.split("_", 4)  # максимум 5 частей
    if len(parts) != 5:
        await callback.answer("Ошибка формата")
        return
    _, _, _, category_key, item_id = parts
...
async def payment_bonus(callback: CallbackQuery, state: FSMContext):
    ...
    if user["bonuses"] > 0:
        ...
        await state.update_data(total_after_bonus=total_after_bonus, bonus_used=bonus_used)
